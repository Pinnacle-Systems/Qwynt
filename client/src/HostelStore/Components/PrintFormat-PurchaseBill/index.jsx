...
export default function PrintFormat({
  ...
  const discount =
    parseFloat(data.ourPrice || 0) - parseFloat(totalAmount || 0);
  const totalOverallCharge =
    parseFloat(data.icePrice || 0) +
    parseFloat(data.packingCharge || 0) +
    parseFloat(data.labourCharge || 0) +
    parseFloat(data.tollgate || 0) +
      ...
      <Page style={styles.page}>
        ...
        <View style={styles.container}>
            ...
            <View style={styles.fromInfoContainer}>
              ...
              <Text style={styles.infoText}>
                <Text style={styles.bold}>
                  FISH MERCHANT & ALL FISH COMMISSION AGENT
                </Text>
              </Text>
              <Text style={styles.infoText}>
                29, KANGEYAM CROSS ROAD, TIRUPUR-4
              </Text>
              <Text style={styles.infoText}>
                8220468448, 8300022602, 9940806009, 6381644996
              </Text>
            </View>
            <View style={styles.toInfoContainer}>
              <Text style={styles.infoText}>
                <Text style={styles.bold}>Supplier Name</Text>:{" "}
                {data?.supplier?.name || "N/A"}
              </Text>
              <Text style={styles.infoText}>
                <Text style={styles.bold}>Contact Number</Text>:{" "}
                {data?.supplier?.contactMobile || "N/A"}
              </Text>
              <Text style={styles.infoText}>
                <Text style={styles.bold}>Contact Person</Text>:{" "}
                {data?.supplier?.contactPersonName || "N/A"}
              </Text>
              <Text style={styles.infoText}>
          ...
          <View style={styles.tableContainer}>
            ...
            {(poBillItems || []).map((item, index) => (
              <View
                key={index}
                style={[styles.tableRow, index % 2 !== 0 && styles.tableRowOdd]}
              >
                <Text style={styles.tableCellitem}>
                  {id
                    ? item?.Product?.name
                    : findFromList(item?.productId, "name") || "N/A"}
                </Text>
                ...
                <Text style={styles.tableCell}>
                  {/* Add content here or keep it empty */}
                </Text>
              </View>
            ))}
          ...
          <View style={styles.table}>
            ...
            <View style={styles.chargeTableRow}>
              ...
              <View style={styles.tableCol}>
                ...
                <Text style={styles.textValue}>{totalOverallCharge}</Text>
              </View>
              <View style={styles.tableCol}>
                <Text style={styles.textLabel}>Discount</Text>
                <Text style={styles.textValue}>
                  {parseFloat(data.ourPrice || 0) - parseFloat(totalAmount)}
                </Text>
              </View>
            </View>
            <View style={styles.footerce}>
              ...
              <View style={[styles.footerRow, styles.highlightRow]}>
                <Text style={[styles.label, styles.highlightText]}>
                  Total Amount:
                </Text>
                <Text style={[styles.value, styles.highlightText]}>
                  {(
                    parseFloat(totalAmount) +
                    parseFloat(totalOverallCharge) +
                    parseFloat(discount)
                  ).toFixed(2)}
                </Text>
              </View>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.footerText}>
            Contact us: fishmarinelife786@gmail.com
          </Text>
